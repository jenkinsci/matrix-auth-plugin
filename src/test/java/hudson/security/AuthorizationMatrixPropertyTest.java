...
public class AuthorizationMatrixPropertyTest {
    ...
    public void testSnippetizer() throws Exception {
        AuthorizationMatrixProperty property = new AuthorizationMatrixProperty(Collections.emptyMap(), new InheritParentStrategy());
    ...
    public void testSnippetizerInapplicablePermission() throws Exception {
        ...
        property.add("hudson.model.Hudson.Administer:dave"); // the important line for this test, inapplicable permission
        ...
        Assert.assertTrue(l.getMessages().stream().anyMatch(m -> m.contains("Tried to add inapplicable permission") && m.contains("Hudson,Read") && m.contains("carol")));
        Assert.assertTrue(l.getMessages().stream().anyMatch(m -> m.contains("Tried to add inapplicable permission") && m.contains("Hudson,Administer") && m.contains("dave")));
    }
    ...
    public void testPipelineReconfiguration2() throws Exception {
        ...
        project.setDefinition(new CpsFlowDefinition("properties([authorizationMatrix(inheritanceStrategy: nonInheriting(), " +
                "permissions: ['USER:hudson.model.Item.Read:bob', 'USER:hudson.model.Item.Configure:bob', 'USER:hudson.scm.SCM.Tag:bob'])])", true));
        j.buildAndAssertSuccess(project);
